...
/*
	settings:
		downloadedCallback: void async function(song) (optional)
		saveFile: string (optional)
		max: number (10000) (optional)
	if saveFile is provided then the songs list is saved to the given file
*/
exports.download = async function(settings) {
	const max = settings.max || 10000;
	const maxTries = settings.maxTries || 10;
	let songs = [];
		...
		promises.push(new Promise(async (resolve, reject) => {
			...
			for (let attempt = 1; attempt < maxTries; attempt++) {
				try {
					...
					if (settings.retryCallback) {
						await settings.retryCallback({
							songs: songs,
							error: e,
							attempt: attempt,
							index: i,
			...
			if (settings.downloadedCallback) {
				await settings.downloadedCallback({
					songs: songs,
					response: response,
					index: i,
				});
	...
	if (settings.saveFile) {
		await fs.writeJson(settings.saveFile, songs);
	}
	return {
		total: initResponse.total,
		songs: songs,
	};
};
...
/*
	settings:
		...
		loadFile: string
		saveFile: string (optional)
		calculateScore: int async function(song) (optional)
		filter: bool async function(song) (optional)
		pointsPerDay: number (1.25/30) (optional)
		playedBase: number (10) (optional)
		scoreBase: number (2) (optional)
		songFormat: string (optional)
		songsFolder: string (optional)
		minDate: Date (optional)
		maxDate: Date (optional)
		display: bool (optional)
		maxDisplay: int (100) (optional)
	Either songs or loadFile (with a valid songs json file at the location) is required.
*/
exports.sort = async function(settings) {
	let songs;
	if (settings.songs) {
		songs = settings.songs;
	} else if (settings.loadFile) {
		songs = await fs.readJson(settings.loadFile);
	}

	if (!songs) {
		throw "songs or loadFile is required";
	}
	...
	const songsFolder = settings.songsFolder;
	const filter = settings.filter;
	const baseFilter = async function(song) {
		if (songsFolder) {
		...
		if (settings.minDate) {
			if (new Date(song.createdAt.date) < settings.minDate) {
				return false;
			}
		}
		if (settings.maxDate) {
			if (new Date(song.createdAt.date) > settings.maxDate) {
				return false;
			}
		}
		return true
	...
	for (let song of songs) {
		...
		song.createdAtUnix = unixTimestamp
		...
		let orderScore = await calculateScore(song);
	...
	if (settings.display) {
		const maxDisplay = settings.maxDisplay || 100;
		const songFormat = settings.songFormat || "%4d: %60s%1s%40s %10s %+5d %+5d = %-5d %5d %7d %7d";
		console.log(sprintf("%-48s %57s %11s %5s %5s %7s %5s %7s %8s", "Index", "Title", "Created", "Up", "Down", "Total", "Pl/d", "Plays", "Download"));
		let index = 0;
		let now = moment();
		for (let song of songs) {
			if (await baseFilter(song) && (!filter || await filter(song))) {
				let days = moment(song.createdAt.date).fromNow(true);
...
exports.playlist = async function(settings) {
	let songs;
	if (settings.songs) {
		songs = settings.songs;
	} else if (settings.loadFile) {
	...
	let listSongs = [];
	for (let i = 0; i < Math.min(songs.length, settings.count || songs.length); i++) {
		listSongs.push({
			key: songs[i].key,
	...
	let playlist = {
		playlistTitle: settings.title || "Autogenerated Playlist",
		playlistAuthor: settings.author || "Auto",
		image: settings.image || defaultImage,
		songs: listSongs,
	};

	if (settings.saveFile) {
		await fs.writeJson(settings.saveFile, playlist);
	}
